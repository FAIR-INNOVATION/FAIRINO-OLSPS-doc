...
该功能用于快速记录机器人当前点位，点击该按钮后，工程树中的“机器人”节点下会新增一个点位targetX，X的作用是防止新增点位重名，如图3-22所示。该点位的j1、j2、j3、j4、j5、j6、x、y、z、rx、ry、rz信息为机器人当前的关节坐标和笛卡尔坐标。
...
指令反馈区。分为工作目录和终端两个选项，其中工作目录显示如图3-31；终端显示程序指令执行结果和返回的机器人错误信息，如图3-22。

.. figure:: analysis/22.png
	:align: center
	:width: 5in

.. centered:: 图3-22 AIRLab指令反馈区
...
该区域包括工具坐标系、工件坐标系、工具坐标系相对于参考坐标系以及长按点动触发、移动步长和转动步长设置四部分内容，如图3-23所示。

.. figure:: analysis/23.png
	:align: center
	:width: 2.5in

.. centered:: 图3-23  AIRLab操作区-笛卡尔空间移动
...
- 长按点动触发、移动步长和转动步长设置部分。如图3-24所示，如果当前导入的机器人模型是实体机器人，长按X+按钮，实体机器人会执行X+点动指令；如果当前导入的机器人模型不是实体机器人，长按X+按钮，仿真机器人会执行X+点动指令。
...
.. centered:: 图3-24  AIRLab操作区-长按点动

关节空间空间移动
~~~~~~~~~~~~~~~~~~~

该区域包括关节J4-J6的12个关节坐标长按触发按钮，6个关节坐标变化文本框以及6个关节滑块三部分内容，如图3-25。

.. figure:: analysis/25.png
	:align: center
	:width: 3in

.. centered:: 图3-25  AIRLab操作区-关节空间空间移动
...
该部分包括“exaxis+”、“exaxis-”以及步长设置框，如图3-26。“exaxis+”、“exaxis-”的功能与工具坐标系下的点动X+、X-类似，通过上述两个按钮可以控制扩展轴的运动。长按按钮控制扩展轴运行，若在扩展轴运行中松开按钮，扩展轴会立即停止运动；若一直按住按钮不松开，扩展轴会运行步长设置框设置的值后停止运动。

.. figure:: analysis/26.png
	:align: center
	:width: 3in

.. centered:: 图3-26  AIRLab操作区-移动扩展轴位置
...
.. centered:: 图3-48  添加拍照节点

添加模型构建节点：添加多组“移动+拍照”节点后添加模型构建节点，新增模型构建节点下编辑无模型工件名称，点击“确定”按钮，无模型模块下出现“模型构建”节点。即添加无模型构建节点成功。

.. figure:: analysis/49.png
	:align: center
	:width: 6in

.. centered:: 图3-49  添加模型构建节点
...
.. centered:: 图3-53  焊缝编辑页面

点击焊缝编辑下的加号弹出焊缝选择弹窗，选择焊缝序号及焊缝类型，点击“确定”按钮添加焊缝，直到所有焊缝添加完成。
...
选择焊缝，点击编辑图标，对焊缝进行编辑，编辑完成之后点击“完成”按钮；直到所有焊缝编辑完成。

.. figure:: analysis/55.png
	:align: center
	:width: 6in

.. centered:: 图3-55   对焊缝进行编辑
...
.. centered:: 图3-56  工件定位添加节点弹窗

工件定位模块程序包含3种节点：拍照、移动、粗定位；拍照和移动节点功能和无模型构建模块拍照和移动功能相同，具体介绍见3.5.2无模型构建部分。
添加粗定位节点：添加多组“移动+拍照”节点后添加粗定位节点，选择工件定位算法后点击确定，工件定位程序下生成“粗定位”节点。

.. figure:: analysis/57.png
	:align: center
	:width: 6in

.. centered:: 图3-57  添加粗定位节点
...
关于
~~~~~~~~~~~~~~~~~~~
选中“关于”，点击该按钮会查询到当前AIRLab软件、中间层和视觉的版本信息和发布日期，如图3-70所示。

.. figure:: analysis/70.png
	:align: center
	:width: 3in

.. centered:: 图3-70  AIRLab版本信息和发布日期显示

日志
~~~~~~~~~~~~~~~~~~~
日志用于记录系统的运行过程及异常信息，可以快速定位问题。点击该按钮会出现一个“日志等级”弹窗。日志记录分为4个等级，分别是INFO、WARNING、ERROR、DEBUG，选择日志等级后设置当前日志等级（默认INFO）。如图3-71所示，具体含义见表3-2。

.. figure:: analysis/71.png
	:align: center
	:width: 2.5in

.. centered:: 图3-71  AIRLab菜单栏-日志
...
.. centered:: 图3-74  选择AIRLab.tar.gz升级包
...
.. centered:: 图3-78  AIRLab软件升级失败及回滚

版本校验
~~~~~~~~~~~~~~~~~~~
点击“窗口”-“版本校验”，弹出版本校验弹窗。如果所有版本都是绿色打勾状态则说明版本校验成功，AIRLab软件可以正常运行，如下图所示。

.. figure:: analysis/version_check.png
	:align: center
	:width: 4in

.. centered:: 图3-79  “版本校验”弹窗

如果遇到某些库是红色打叉状态，则需要检查AIRLab环境版本是否对应，本此以中间层库版本为例，如下图所示。

.. figure:: analysis/version_check_error.png
	:align: center
	:width: 4in

.. centered:: 图3-80  “版本校验”有误
...
.. centered:: 图3-88  开启碰撞检测规划的自动避障路径

.. figure:: analysis/86.png
	:align: center
	:width: 4in

.. centered:: 图3-89  不开启碰撞检测规划的空移路径
...
点击菜单栏工艺-圆柱填充，AIRLab界面显示圆柱填充工艺查询弹窗。图3-92所示为圆柱填充工艺查询的弹窗，圆柱填充工艺包括圆柱底面填充和二次加固两个部分。

.. figure:: analysis/93.png
	:align: center
	:width: 3in

.. centered:: 图3-96  圆柱填充工艺工艺查询弹窗
...
.. centered:: 图3-100  达到清枪剪丝周期，AIRLab询问弹窗
...
.. centered:: 图3-101  自动模式下未设置周期的提示弹窗
...
.. centered:: 图3-102  AIRLab菜单栏-窗口-自动循环运行
...
.. centered:: 图3-104  工件正在识别中

.. figure:: analysis/102.png
	:align: center
	:width: 6in

.. centered:: 图3-105   工件识别成功，匹配度为82%

.. figure:: analysis/103.png
	:align: center
	:width: 6in

.. centered:: 图3-106  自动检索焊接工程后导入新工程

.. figure:: analysis/104.png
	:align: center
	:width: 6in

.. centered:: 图3-107  工件识别失败提示信息
...
.. centered:: 图3-108   一次循环运行结束，正在等待循环间隔
...
.. centered:: 图3-109  达到设置的循环次数，结束自动循环运行
...
用户数据备份
~~~~~~~~~~~~~~~~~~~
如果用户需要将一台设备上已配置好的焊接工艺，模板程序及构建的工件数据导入到其他设备，实现环境复刻，AIRLab提供了用户数据备份功能。
...
.. centered:: 图3-110  扩展轴设置弹窗

导出功能：点击“导出”按钮，AIRLab首先会将当前软件使用数据包的版本写入version.txt文档中，以便导入时进行版本匹配校验，之后AIRLab开始复制下面的数据内容：可执行文件所在目录下的Data文件夹下存储工件配准模板的文件夹Work_template，存储焊接模板程序的文件夹Weld_template，存储工件及工具模型的文件夹entity，存储用户创建的焊接工艺数据的数据库文件Airlab_weld_peocess.db；主目录的data文件夹下的output模型，自动保存模型文件register_model ，焊缝数据库weld_seam_database三个文件夹。
...
.. centered:: 图3-111  用户数据正在打包导出中

.. figure:: analysis/109.png
	:align: center
	:width: 6in

.. centered:: 图3-112  用户数据导出完成
...
.. centered:: 图3-113  数据包的目录结构

导入功能：点击“选择文件”按钮，选择需要导入的数据包（要导入的数据包目录请与图3-110中的结构保持一致），然后点击“导入”按钮，AIRLab首先会校验导入数据包中的version.txt中的版本号，如果版本号一致则会开始数据包内容的导入；如果版本号不匹配，会给出弹窗信息提示用户版本号不一致，数据会不兼容无法进行导入。

.. figure:: analysis/111.png
	:align: center
	:width: 6in

.. centered:: 图3-114  选择待导入的数据包

.. figure:: analysis/112.png
	:align: center
	:width: 6in

.. centered:: 图3-115  数据包正在导入中

.. figure:: analysis/113.png
	:align: center
	:width: 6in

.. centered:: 图3-116  数据包导入完成

.. figure:: analysis/114.png
	:align: center
	:width: 6in

.. centered:: 图3-117  数据包无法导入提示
...
.. centered:: 图3-128  扩展轴设置弹窗

在需要扩展轴联动的长直焊缝编辑时，设置起点和终点的扩展轴位置，如图 3-129所示。在焊缝识别后，AIRLab会根据焊缝识别结果和扩展轴位置自动生成程序，实现扩展轴和机器人的同步运动。

.. figure:: analysis/116.png
	:align: center
	:width: 3in

.. centered:: 图3-129  长直焊缝编辑时需要设置扩展轴位置参数
...
.. centered:: 图3-132 仿真页面

程序配置
~~~~~~~~~~~~~~~~~~~

进行程序运行配置和程序起弧配置，如图3-133。

.. figure:: analysis/120.png
	:align: center
	:width: 3in

.. centered:: 图3-133  程序配置页面
...
AIRLab软件目前提供了中文（简体）、中文（繁体）、英文、日文、韩语、俄语、法语七种语言选择，详细的多语言设置页面如图3-137所示。该页面提供三种操作：切换语言；导出AIRLab软件中已有的语言；导入新语言。满足用户切换多语言，为AIRLab软件设置新语言以及修改AIRLab软件已有的语言内容的需求。

.. figure:: analysis/124.png
	:align: center
	:width: 3in

.. centered:: 图3-137 “多语言设置”页面

上述功能的详细操作介绍如下：

1. 切换AIRLab的语言

点击图3-115中“多语言选择”的下拉框，选择需要的语言类型后点击“确定”按钮即可立即切换AIRLab软件语言。
...
.. centered:: 图3-138 AIRLab语言文件导出路径
...
.. centered:: 图3-139 AIRLab语言CSV文件的内容及格式

如果使用LibreOfffice软件打开，如图3-118，注意打开格式如下图3-140所示。

.. figure:: analysis/127.png
	:align: center
	:width: 2.5in

.. centered:: 图3-140  LibreOffice软件

.. figure:: analysis/128.png
	:align: center
	:width: 6in

.. centered:: 图3-141  AIRLab多语言文件的打开格式

其次是编写用户的CSV文件，用户设置新语言时只需要修改第一列language_id和第四列translation_text的内容。假设用户新增的是法语，就使用“Français”替换图3-117中第一列下的所有“English”；第四列translation_text的内容需要用户根据“source_text”的中文文本进行翻译得到对应的目标语言(对于源文本中出现的相同字符串，请将其翻译为同样的单词)。

.. important:: 
	请不要修改“source_text”列下的任何字符！

完成翻译工作后，用户需要为该CSV文件重命名一个文件名，文件名将是AIRLab语言数据库中的该语言数据表的表名，比如图3-116中的文件名“en_translation_table”就是语言类型“English”在数据库中的表名。

.. important:: 
	用户CSV文件的命名建议保留其语言特色，避免与数据库中已有的语言数据表名重复，由此导致其他语言数据表内容被替换的错误。

最后是向AIRLab软件中导入该CSV文件，将该文件复制到~/AIRLabExe/CSVfile目录下，点击“导入”按钮，选择该文件导入，如图3-142。AIRLab终端显示“CSV文件导入成功”即表示用户的语言文件导入成功，如图3-143。重启AIRLab后，在“语言选择”的下拉框选择用户新增的语言切换即可。

.. figure:: analysis/129.png
	:align: center
	:width: 4in

.. centered:: 图3-142  “导入”按钮的弹窗

.. figure:: analysis/130.png
	:align: center
	:width: 3in

.. centered:: 图3-143 语言文件导入成功时的终端显示信息

如果终端显示“CSV文件导入失败”，可以查看日志记录的错误信息，如图3-122所示，并仔细检查导入的CSV文件与最初导出的CSV文件是否在行数、列数以及列之间的中文分隔符“；”有不一致的地方。

.. figure:: analysis/131.png
	:align: center
	:width: 6in

.. centered:: 图3-144  语言文件导入失败时的日志信息提示
...
.. centered:: 图3-148  扩展轴坐标系标定界面（左）与弹窗提示（右）
