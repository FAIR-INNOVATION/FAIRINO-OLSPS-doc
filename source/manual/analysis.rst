...
现有的插件包括焊接插件、样条曲线插件、bin picking插件、喷涂插件及对话式智能助手插件五个功能模块。可以选择打开或关闭插件，同时可以通过“插件授权”查看各个插件的授权状态以及对各个插件进行授权。各插件的详细介绍与具体操作，请参见第五章插件部分。
...
焊接
~~~~~~~~~~~~~~~~~~~
在“焊接”主功能下，设有九个二级选项，分别为：“焊接程序配置”、“焊接数据采集”、“全局设置”、“清枪剪丝”、“自动循环运行”、“用户数据备份”、“3D文件解析”、“多工位自动运行”及“焊丝干伸长度补偿”。点击任意选项，AIRLab将弹出对应的焊接功能设置窗口。各项功能的具体说明与操作方法，详见3.6节中的弹窗介绍。
...
用户需要根据实际工件特征点击“确认使用”或者“重选特征”按钮，重选特征的详细使用方法见章节3.6.25。
...
- 地面效果验证：对所拍摄计算的地面进行可视化验证，细操作见2.6章节地面获取与验证；

SLAM建图
~~~~~~~~~~~~~~~~~~~
首先点击工程模块的SLAM建图模块，对整个过程采用的方式和拍照进行设置。整个过程提供两种传感器选择，为“相机”和“雷达”，但由于目前雷达方式还未完成，故只提供相机SLAM建图的方式。扫描方式有两种，分别为“摆动扫描”和“固定扫描”，下面对整个SLAM建图过程进行主要步骤分解。

1.摆动扫描

首先点击SLAM建图模块-相机标志打开“SLAM建图拍照设置”界面，如果选择模式为摆动扫描，则按照如下图进行扫描方式设置。
...
完成模式设置后，准备进行SLAM建图程序编写，打开“SLAM建图”界面，同时注意由于是摆动扫描模式，在添加移动节点时，请使用界面靠上方的点位选择按钮进行添加。

.. figure:: analysis/slam_map.png
	:align: center
	:width: 3in

	摆动-节点添加

在该模式下，程序由多组移动+拍照节点组合；原则是要将箱体模型拍摄完整；所有的移动+拍照节点添加完成后，添加SLAM建图节点，程序完成结构如下图所示。

.. figure:: analysis/slam_lua1.png
	:align: center
	:width: 3in

	摆动-程序编写

完成程序编写后点击“SLAM建图”标题，再点击运行程序，等待程序运行完成。

.. figure:: analysis/slam_lua1_run.png
	:align: center
	:width: 3in
...
如果选择模式为固定扫描，则按照如下图进行扫描方式设置，并且针对该模式有两种操作方式。

.. figure:: analysis/slam_photo_set2.png
	:align: center
	:width: 3in
...
(1)手动模式
该操作模式下，点击“SLAM建图拍照设置”界面中的“开始扫描”按钮启动，拖动机械臂运动，持续扫描箱体建模。当遇到箱体较大需要过渡时，点击“暂停扫描”后拖动至下一个目标扫描点，点击“恢复扫描”继续操作。直到整个箱体都完成扫描，依次点击“暂停扫描”“结束扫描”“获取扫描结果”按钮从而获取SLAM建图结果。
(2)自动模式
若为自动模式，则打开“SLAM建图”界面，首先点击“点位类型”选择移动节点的类型，再进行点位选择以及确认。
...
在该模式下，程序由多组包含起点、终点和过渡点的移动指令组成；原则是要将箱体模型拍摄完整；所有的移动节点添加完成后，添加SLAM建图节点，程序完成结构如下图所示。过渡点不是必须设置的，但是每条焊缝要有起点和终点。

.. figure:: analysis/slam_lua2.png
	:align: center
	:width: 3in
...
.. figure:: analysis/slam_lua2_run.png
	:align: center
	:width: 3in

	固定-自动模式-运行程序

无论采用何种方式，SLAM建图成功后，将会在三维场景中显示获取到的完整的箱梁模型，如下图所示。

.. figure:: analysis/slam_res.png
	:align: center
	:width: 6in

	SLAM建图结果

若上述步骤完成SLAM建图程序运行后得到的模型不完整，则可以再对模型进行补拍。点击SLAM模块下相机图标，再次打开“SLAM建图拍照设置”页面，如下图所示。
...
考虑到有摆动扫描、固定扫描(手动)和固定扫描(自动)三种建图方式，现也提供相应的三种补拍方式。

1.摆动扫描补拍

找到上述界面中部的“摆动扫描补充拍摄”部分，确定想要补拍的点位后，移动至该点后，点击“补充扫描”。重复上述步骤，直到所有补拍都完成后，点击“重新SLAM建图”后，获取建图结果。

2.固定扫描(手动)补拍

找到上述界面最上部的“开始补充扫描”按钮，点击后即可拖动机器人进行补拍。补拍完成后，依次点击“暂停扫描”“结束扫描”“获取扫描结果”按钮从而获取SLAM建图结果。

3.固定扫描(自动)补拍

找到上述界面中下部的“固定扫描补充拍摄”部分，选择想要补拍路径的起点和终点，完成点位选择后点击“补充扫描”按钮即可补拍。完成所有补拍后，点击“重新SLAM建图”即可重新获取建图结果。

若后续开启避障轨迹功能，则需要获取目标物体的碰撞模型。整个模型获取过程需进行拍照获取目标的完整数据，若之前已进行SLAM建图扫描获得一部分数据，只需对没拍的部分进行相应补拍即可。当完成整体目标物体的拍照过程后，打开“全局设置”页面(见3.6.6章节)，点击“获取碰撞模型”按钮即可得到目标物体的碰撞模型，如下图所示。

.. figure:: analysis/slam_collision_model.png
	:align: center
	:width: 4in

	目标物体碰撞模型

完成SLAM建图后，需要对精度进行验证。打开“SLAM建图拍照设置”界面，点击最下方的“开始验证”按钮，移动至箱梁模型的一角，点击“验证拍照”按钮，再移动到其对角，再次点击“验证拍照”按钮后，完成箱梁模型的两个对角拍摄。最后点击“获取验证结果”按钮，即可获取结果。

如果验证结果通过，则会显示弹窗，如下图所示。

.. figure:: analysis/slam_verify_res_good.png
	:align: center
	:width: 3in

	SLAM精度验证通过

反之验证结果不通过，则会显示弹窗，如下图所示。请检查各项参数后重新建图。

.. figure:: analysis/slam_verify_res_bad.png
	:align: center
	:width: 3in
...
可以选择新增无模型构建参数节点、新增拍照节点、新增移动节点或新增模型构建节点；
...
添加模型构建节点：添加多组“移动+拍照”节点后添加模型构建节点；首先选择模型构建方法，包括直线＋圆弧和样条曲线两种方法，选择样条曲线则需要设置采样间隔；模型构建方法选择完成后，编辑无模型工件名称。点击“确定”按钮，无模型模块下出现“模型构建”节点。即添加无模型构建节点成功。
...
程序配置页面是在运行程序之前对程序运行进行配置，包括程序配置部分和焊接中断恢复配置部分；如图3-128。
...
如果模型构建方式选择的是“相机采集”或者“3D文件继承”，请继续点击“下一步”，进入平面特征选择页面，如下图所示。
...
如果是相机采集的平面特征选择，考虑到目前四种平面特征的优先级特点，所以选择该平面特征时，需要点击“重选特征”按钮，根据工件的平面结构按照界面给出的交互提示选择“是”或者“否”，如下图所示，已选择的特征会出现在页面中“已选择特征”下方的列表中。
...
.. important::
	该页面的图标按钮交互方式与其他特征不同，点击后只能查看焊接特征描述弹窗，并不能执行选择操作，选择特征必须点击“重选特征”按钮才能生效！

如果是3D文件继承的平面特征选择，目前有3D普通平面和3D箱梁平面两种。点击图标按钮可以查看焊接特征描述弹窗，同时该特征会出现在页面下方的在“已选择特征”列表中。如下图所示。
...
平面特征选择结束后，点击图 3-219或者图 3-220中的“下一步”按钮，进入圆柱与圆锥特征选择页面，如下图所示。矮圆柱、高圆柱、高矮圆柱共存三种特征需要实际测量圆柱半径后在文本框中输入圆柱的最大半径和最小半径值。如果当前工件不涉及圆柱与圆锥特征，点击“取消选中”后再点击“下一步”，如果当前未选择圆柱与圆锥特征，直接点击“下一步”即可。
